...
This also uses part of Erlice's aperture os repository (hence the apertureosnew name) because it was last updated 8 years ago and also uses neofetch so that's not cool. We will be using fastfetch, and making simple changes to the config file. This means that you have to generate a config.jsonc file. (if you dont know how to do this, the commands will be down below, or, just use my demonstration config :p)
